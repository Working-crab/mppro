...
class Process(multiprocessing.Process):
    ...
    async def parse_search(self, session, page, search_word):
        url = f'https://search.wb.ru/exactmatch/ru/common/v4/search?page={page}&resultset=catalog&TestGroup=no_test&TestID=no_test&appType=1&curr=rub&dest=-1257786&query={search_word["id"]}&regions=80,38,4,64,83,33,68,70,69,30,86,75,40,1,66,110,22,31,48,71,114&resultset=catalog&sort=popular&spp=0&suppressSpellcheck=false'
            ...
            try:
                ...
                for i in range(len(hashrate.get('data',{}).get('products',''))):
                    result = {
                        ...
                        "search_word" : search_word["id"],
                        "position_word" : search_word["position"]
                    }
    ...
    def db_insert_info(self, result_scraping):
        try:
            query = "INSERT INTO product_position (`id`,`id_product`,`position`,`search_word`,`date_collected`,`position_word`) SETTINGS async_insert=1,wait_for_async_insert=1 VALUES "
            values_list = []
            for i2 in result_scraping:
                for i in i2:
                    if i["search_word"] != None:
                        values_list.append(f"""(generateUUIDv4(),{i["id"]},{i["place"]},'{i["search_word"].replace("'","''")}','{self.date_collected}','{i["position_word"]}')""")
            add_list = []
